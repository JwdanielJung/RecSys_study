...
if __name__ == "__main__":
    ...
    # 모든 weight 불러오기 & 초기화 시 xavier 활용
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step in range(
            ...
            # pos는 label 1, neg는 0으로 만들기
            pos_labels, neg_labels = torch.ones(
                pos_logits.shape, device=args.device
            ), torch.zeros(neg_logits.shape, device=args.device)

            adam_optimizer.zero_grad()  # 초기화 필수!

            indices = np.where(pos != 0)  # pad_idx인 0 제외하기
            loss = bce_criterion(pos_logits[indices], pos_labels[indices])
            loss += bce_criterion(
                neg_logits[indices], neg_labels[indices]
            )  # neg도 pad아닌 곳에서만 있기 때문에 동일 indices 사용

            # L2 정규화 추가 시 loss에 더하기 (default는 0)
